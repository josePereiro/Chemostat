...
function add_costs(metnet::MetNet, cost_info::Dict; 
    ...
    metnet2 =  add_rxn(metnet_with_costs, cost_rxn_id,
        mets = Dict(cost_met_id => 1.0),
        lb = cost_rxn_lb, ub = cost_rxn_ub, 
        rxnName = cost_rxn_name)
    verbose && println("rxn '$(cost_rxn_id)' added, $(rxn_str(metnet2, cost_rxn_id)), bounds $(bounds(metnet2, cost_rxn_id))")
    verbose && println("met '$(cost_met_id)' added, $(balance_str(metnet2, cost_met_id))")
    return metnet2
end