function plot_stoi_err_beta!(p, boundle::ChstatBoundle, 
    ...
    labeled = false
    try
        errs = [av_stoi_err_ep(boundle, ξ, β, ider) for β in βs]
        plot!(p, βs, errs; label = label, lw = lw, kwargs...)
        labeled = true
    catch KeyError end
    try
        label = !labeled ? label : ""
        errs = [av_stoi_err_hr(boundle, ξ, β, ider) for β in βs]
...
function plot_stoi_err_beta!(p, boundle::ChstatBoundle, 
    ...
    for (ider, color) in zip(iders, colors)
        plot_stoi_err_beta!(p, boundle, ξ, βs, ider; 
